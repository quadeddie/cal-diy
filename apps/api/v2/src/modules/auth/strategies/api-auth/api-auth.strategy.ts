...
export class ApiAuthStrategy extends PassportStrategy(BaseStrategy, "api-auth") {
  ...
  async apiKeyStrategy(apiKey: string, request: ApiAuthGuardRequest) {
    // DojoNext patch: cal.diy v6.2.0 still enforces the CALCOM_LICENSE_KEY EE
    // gate here despite being branded MIT. We drop the check locally because
    // cal.diy is MIT-licensed and the gate is not enforceable on us. Re-apply
    // this patch on cal.diy upgrades; see docs/research/cal-diy-integration.md.
